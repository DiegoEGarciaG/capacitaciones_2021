...
def det_duckie(obs):
    ...
    image_out = cv2.erode(image_out, kernel, iterations = 2)
...
if __name__ == '__main__':
    ...
    C = 1 #(f es constante, dr es conocido)
    ...
    while True:
        ...
        dist = (dbot_pos - duck_pos)
        dist = (dist[0]**2 + dist[1]**2 + dist[2]**2)**(1/2)
        ...
        for d in dets:
            ...
            # Si fuera presencial se debería ver la distancia de frenado en función de velocidad no?
            if d_aprox < 0.3:
                # Activar alarma
                alert = True
